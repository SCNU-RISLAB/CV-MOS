...
class double_conv_circular(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.pad(x, (1, 1, 0, 0), mode='circular')  # 这四个值分别对应左、右、上、下四个方向的填充量
        x = self.conv1(x)
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, circular_padding, bilinear=False, group_conv=False, use_dropblock=True,
                 drop_p=0.5):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_ch, in_ch // 2, 2, stride=2, groups=in_ch)
        else:
