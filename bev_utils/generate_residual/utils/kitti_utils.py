...
def polar_projection(current_vertex, proj_H=360, proj_W=480, max_range=50.0, min_range=2.0, max_z=2.0, min_z=-4.0,
    ...
    scan_z = current_vertex[:, 2]  # 极坐标的(r, θ, z)
    ...
    order_max = np.argsort(z_valid)  # 排序并提取索引，z较小值会被较大值覆盖
    ...
    order_min = order_max[::-1]  # z较大值会被较小值覆盖
    ...
    occlusion_mask = np.full(proj_z_delta.shape, False, dtype=bool)  # True表示被遮挡
    if return_occlusion:
